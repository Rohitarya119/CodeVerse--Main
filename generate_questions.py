...
# Generate SQL
sql_content = "INSERT INTO problems (title, description, difficulty, topics, constraints, boilerplate_code, function_name, examples) VALUES \n"
values_list = []
...
for q in questions:
    ...
    constraints = json.dumps(q["constraints"]).replace("'", "''")
    boilerplate = json.dumps(q["boilerplate_code"]).replace("'", "''")
    func_name = q["function_name"]
    examples = json.dumps(q["examples"]).replace("'", "''")
    
    value = f"('{title}', '{desc}', '{diff}', '{topics}'::jsonb, '{constraints}'::jsonb, '{boilerplate}'::jsonb, '{func_name}', '{examples}'::jsonb)"
    values_list.append(value)

sql_content += ",\n".join(values_list) + ";"

# Write to file
with open("insert_two_pointers_30.sql", "w", encoding='utf-8') as f:
    f.write(sql_content)

print("SQL file generated successfully with " + str(len(questions)) + " questions.")
